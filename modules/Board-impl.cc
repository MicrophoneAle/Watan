...
import <string>; // Needed for std::to_string
...
Board::Board() {
    ...
    for (int i = 0; i < 72; ++i) goals[i].setId(i);*/
...
void Board::display(std::ostream& out) const {
    ...
    auto c = [&](int i) { return "|" + criteria[i].getDisplayStr() + "|"; };
    auto h = [&](int i) { return "--" + goals[i].getDisplayStr() + "--"; };
    auto d = [&](int i) { return goals[i].getDisplayStr(); };
    auto t = [fmt](int i) { return fmt(i); };
    auto r = [&](int i) { return tiles[i].getResourceStr(); };
    auto v = [&](int i) { return tiles[i].getValueStr(); };
    */
    ...
    auto t = [fmt](int i) { return fmt(i); }; // INDEX NUMBERS
    auto r = [](int i) { return "CAFFEINE"; }; // RESOURCE NAMES
    auto v = [](int i) { return "XX"; }; // DICE NUMBERS

    // --- EXACT INDENTATION LEVELS ---
    const std::string S1 = "   ";
    const std::string S2 = "                  ";
    const std::string S3 = "                                 ";
